...
class Window(tk.Tk):
    ...
    def set_text(self, new_text):
        ...
        """
        if self.__text_area is not None:
...
class Game:
    ...
    def _get_game_objs(self):
        """Returns a list containing all the GameObjects.
        This should not be called directly;
        Create your own composition and aggregation relationships instead."""
    ...
    def _remove_game_obj(self, obj):
        ...
        while obj in self._gameObjects:
    ...
    def run(self):
        ...
        while self._window.is_open():
            ...
            for object in self._get_game_objs():
...
class Vector2D:
    ...
    def add(self, vec):
        ...
        if type(vec) != Vector2D:
            raise TypeError(f"You are providing the wrong type\n"
                            f"Expecting Vector2D, but {type(vec)} is given")
    ...
    def scale(self, scalar):
        ...
        if type(scalar) != int and type(scalar) != float:
            raise TypeError(f"You are providing the wrong type\n"
                            f"Expecting class int, but {type(scalar)} is given")
...
class Rectangle:
    ...
    def move_by(self, dx, dy):
        ...
        self._position.x += dx
        self._position.y += dy
        if self._window.get_canvas() is not None:
